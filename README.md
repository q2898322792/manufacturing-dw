...
> 数据为仿真生成，结论用于展示「从数仓到分析结论」的完整链路，不代表真实市场。

**经营总览**：统计期内营收 **259.66 亿元**、订单 **113.3 万单**，覆盖 300 家客户、201 款产品，退货率 2.71%。

| # | 发现 | 数据依据 | 建议动作 |
|---|---|---|---|
| 1 | **单均金额异常波动（5 倍）** | 月订单量恒定在 9~10 万单，但单均金额在 7,753 ~ 40,877 元间波动：2025-12 / 2026-01 翻倍至 4 万元+，2026-02 腰斩至 7,753 元 | 需核实是大单/促销所致，还是金额字段异常 —— 若是后者，所有营收类指标均失真 |
| 2 | **客户结构极度分散** | Top 10 客户仅占营收 **3.5%**，Top 20%（60 家）占 20.6% | 无核心大客户依赖，经营风险低；但也缺少大客户抓手，可考虑分层运营 |
| 3 | **区域差异不显著** | 7 大区域营收均衡（35~39 亿元），履约率差异 < 0.7 个百分点 | 履约问题不是区域性的，应转产品 / 客户维度排查 |
| 4 | **库存健康度良好** | 600 个物料×仓库组合中呆滞仅 **11 个（1.8%）**，呆滞占用 **0.48 亿元（4.7%）**，最长周转 118 天 | 维持现有库存策略，重点监控周转天数上升的物料 |
| 5 | **预警集中在资金链** | 「回款滞后」30 条 + 「营收缺口」25 条，合计占预警总量 **70%** | 经营风险主要在回款端而非生产端，建议优先跟催 |

> ⚠️ 口径提示：`ads_sale_analysis` 的履约率按订单数加权为 80.62%，而按「已交付订单数」正确加权为
> 87.99%（差 7.4 个百分点）—— 因为该宽表只有比率列、没有计数列，无法正确加权（见[常见坑](#常见坑只列最容易踩的)）。
