...
- **当前数据范围**：`2025-10-01 ~ 2026-06-29`（**2026 年 7、8、9 月是缺口**，见 §7.1）。
...
> ✅ **文档数字不一致问题已于 2026-09-19 清理**：原先两处数字打架（库存金额 `4,709.6 万元` vs
...
1. ~~**增量脚本的缺口（最高优先级）**~~ —— **已于 2026-09-18 解决**。
...
2. **`README.txt` 是 `README.md` 的未跟踪副本**（2026-09-16 18:56 生成，`git status` 显示 `?? README.txt`）。
   需要决定：删除、或提交、或在 `.gitignore` 里忽略。两份 README 并存迟早会不一致。
3. **`TRUNCATE` 的原子性**：建议改成"写入临时表 → 成功后再原子切换"，或把 TRUNCATE 挪到 INSERT 之后（§4.5）。
4. **MySQL 运维项（未做，降低再次爆盘风险）**：
...
5. **没有 git remote**：18 个 commit 全在本地，无异地备份。
   **⚠️ 推远程前必须先清理 `.git`（2026-09-18 实测）**：`.git` 目录 **57 MB**，而工作区被跟踪文件
   只有 **1.3 MB** —— 差额全是历史垃圾：**6 个 2.7~11.5 MB 的超大 PNG 截图**（`12429×6045` 这种尺寸，
   应是早期 BI 复核用的整页截图，共约 50 MB）**+ 52 个 unreachable blob**。
   经 `git rev-list --objects --all` 核对，这些大对象**全是游离对象、未被任何提交引用**，
   所以 `git gc --prune=now` 即可清除（无需 `filter-branch` / BFG）。
   否则第一次 push 就要上传 57 MB 的垃圾。
   > 教训：**别把整页大截图 `git add` 进仓库**；`_bi_review/` 被忽略不是偶然。
6. **被 `.gitignore` 排除、换机器需要手动搬的产物**：`bl/*.pdf`（7 份看板导出 PDF，约 5.9 MB）、`logs/`（ETL 日志）、
   `.vscode/`、`_bi_review/`（BI 复核临时产物，当前已清理）。**同机换工具不受影响**。
7. ~~**文档轮次混乱**~~ —— **✅ 已于 2026-09-19 解决**：原 `BI看板数据修正说明.md`
